...
void manual()
{
      ...
      void scheduling()
      {
              if(WiFi.status() == WL_CONNECTED && Firebase.get(feed,"/AFFV2") && feed.dataType()=="null") // check if there is the diredtory path in the firebase.
                {
                  Serial.println("Path does not Exist:");
                }
                else if(WiFi.status() == WL_CONNECTED && Firebase.get(feed,"/AFFV2") && feed.dataType()!="null")
                { 
